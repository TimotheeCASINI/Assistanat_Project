...
if selected == "New Data":
    ...
    if uploaded_file is not None:
        ...
        AgGrid(df_new_data)
        st.session_state.list_caracterisation = df_new_data.iloc[[0]].dropna(axis=1).values.flatten()#list_carcterisation => liste des catégories tel que Analyse sensorielle
        ...
        for caract_key in st.session_state.list_caracterisation: #Creation dict_caracterisation, Parcours toutes les catégories
            ...
            list_carac_value = []
            for index in range(caract_key_index,df_new_data.shape[1]):#Parcours toutes les sous catégories à partir de l'id de leur catégories
                value_column = df_new_data.iloc[1, index]
                if pd.isnull(df_new_data.iloc[0, index]) == False and df_new_data.iloc[0, index]!=caract_key:#Break permettant de changer de catégories
                    break
        ...
        list_experience = (st.session_state.dict_caracterisation.values())#Liste experience => liste de toutes les expériences
        ...
        for experience_key in list_experience:#Creation dict_experience
            ...
            for index in range(experience_key_index,df_new_data.shape[1]):
                ...
                if pd.isnull(df_new_data.iloc[1, index]) == False and df_new_data.iloc[1, index]!=experience_key:#Break permettant de changer de sous catégories
                    break
                if pd.isnull(value_column) == False:
        ...
        df_new_data = df_new_data.apply(pd.to_numeric)
        ...
        st.session_state.name_file = name_file
...
if selected == "Visualization Data":
    if not st.session_state.data_load:
        ...
        col = st.columns(3)
        with col[1]:
            st.button("Charger base de données de générale")
        ...
        st.sidebar.button("Sauvegarde des données dans la bdd")